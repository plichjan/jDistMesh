...
val dc = DCircle(0.0, h, r)
val rc = DRectangle0(0.0, w, 0.0, h)
val h0 = 3e-3

var mesh2 = distMesh2D.call(
    DDiff(rc, dc),
    { x, y -> h0 + 0.05 * dc.call(x, y) },
    h0, arrayOf(doubleArrayOf(0.0, 0.0), doubleArrayOf(w, h)),
    pfix
)

val v0Poly = DPoly(listOf(Pnt(0.0, 0.0), Pnt(w, 0.0), Pnt(w, h), Pnt(w, 0.0)))

mesh2.p = BndProj().call(mesh2.p!!, mesh2.t!!, DDiff(rc, dc))
mesh2 = AddMidpoints().call(mesh2.p!!, mesh2.t!!)
viewFrame.drawMesh(mesh2)

storeMesh("${dir}rectangleWithHole", mesh2, h0 / 10.0, listOf(
    Constrain(df = dc,     flags = IS_SET_POTENTIAL,      potential = 1.0),
    Constrain(df = v0Poly, flags = IS_SET_POTENTIAL,      potential = 0.0),
    Constrain(df = DPoly(listOf(Pnt(0.0, 0.0), Pnt(0.0, h))), flags = IS_SET_DISPLACEMENT, displacement = doubleArrayOf(0.0, Double.NaN)),
    Constrain(df = DPoly(listOf(Pnt(0.0, h),   Pnt(w,   h))), flags = IS_SET_DISPLACEMENT, displacement = doubleArrayOf(Double.NaN, 0.0)),
    Constrain(df = DPoly(listOf(Pnt(0.0, 0.0), Pnt(w,   0.0))), flags = IS_SET_BOUNDARY_FORCE, bForce = doubleArrayOf(0.0, -2e10))
), listOf(
    TrConstrain(df = forEvery, matId = 21)
))

println("Done: rectangleWithHole")
